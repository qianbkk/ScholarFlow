...
def _reset_budget_state(monkeypatch):
    """每个测试前重置预算 counter 和 reset_ts，并设置一个小的 GLOBAL_HOURLY_BUDGET 便于测试。"""
    main_mod._budget_counter.clear()
    main_mod._budget_reset_ts = _time.time()  # 不强制重置
...
def test_concurrent_reserves_never_exceed_budget():
    ...
    main_mod._budget_counter["total"] = 0.5  # 预热：剩余 0.5 容纳 5 个 0.1
    ...
    assert main_mod._budget_counter["total"] <= 1.0 + 1e-9, (
        f"counter {main_mod._budget_counter['total']} exceeded budget 1.0"
    )
    # 确认 5 个 0.1 累加后正好 = 1.0
    assert abs(main_mod._budget_counter["total"] - 1.0) < 1e-9


def test_reserve_exactly_at_budget_boundary():
    """当 total == budget - estimated_cost 时，刚好可以预留（边界条件）。"""
    main_mod.GLOBAL_HOURLY_BUDGET = 0.5
    main_mod._budget_counter["total"] = 0.3  # 剩余 0.2
...
def test_sequential_reserve_strictly_accumulates():
    ...
    asyncio.run(run())
    # 0.0 + 0.2 + 0.2 + 0.2 = 0.6
    assert abs(main_mod._budget_counter["total"] - 0.6) < 1e-9


def test_first_reserve_after_window_expiry_resets():
    """时间窗口过期后，第一次 reserve 应自动清零 counter。"""
    main_mod.GLOBAL_HOURLY_BUDGET = 1.0
    main_mod._budget_counter["total"] = 0.9  # 旧窗口累计接近上限
    main_mod._budget_reset_ts = _time.time() - 7200  # 2 小时前
    ...
    asyncio.run(run())
    # 窗口过期 → 清零 → 新窗口累计 0.1
    assert abs(main_mod._budget_counter["total"] - 0.1) < 1e-9
...
def test_reserve_too_large_single_call_fails():
    ...
    # 失败时 counter 不应被污染
    assert main_mod._budget_counter["total"] == 0.0


def test_concurrent_reserves_persist_after_release():
    """并发 reserve 完成后，counter 持久化（_persist_budget_state 调用）应被触发。"""
    main_mod.GLOBAL_HOURLY_BUDGET = 1.0
    main_mod._budget_counter["total"] = 0.0

    persist_calls = [0]
    original_persist = main_mod._persist_budget_state

    def counting_persist():
        persist_calls[0] += 1
        original_persist()

    main_mod._persist_budget_state = counting_persist
    try:
        async def run():
        ...
        main_mod._persist_budget_state = original_persist

    # 3 次 reserve → 3 次 persist（每次都在锁内）
    assert persist_calls[0] == 3
